...
function renderTable(data, onlyLast2) {
    ...
    for (const prizeName in prizes) {
        ...
        regions.forEach((region) => {
            ...
            var value =
                prizeData && prizeData[region]
                    ? formatNumber(prizeData[region], onlyLast2)
                    : "";
...
function searchNumber() {
    ...
    spans.forEach((row) => {
        ...
        const regionIndex = Array.from(cells).findIndex(
            (cell, idx) =>
                idx > 0 && fullDataCache.regions[idx - 1] === province
        );
        if (regionIndex !== -1) {
            ...
            for (const v of values) {
                if (v === number.slice(6 - v.length)) {
                    ...
                    found = true;
                    showAlert(
                        `Chúc mừng bạn đã trúng <b>${cells[0].textContent}</b> số <b class="red">${v}</b>`
                    );
                }
            }
        }
...
function loadData(dateStr) {
    ...
    q("#yearSelect")[0].value = dateParts[2];
    q("#monthSelect")[0].value = dateParts[1] - 1; // month is 0-indexed in JS
    updateDayButtons(parseInt(dateParts[1]) - 1, parseInt(dateParts[2]));
...
function init() {
    ...
    document
        .getElementById("lastTwoCheckbox")
